...
%         end
        ...
        function varargout = skill(obj,varargin)
            ...
            skl{4} = '*/';
            skl{5} = ['printf(" Loading ADEXL Testbench for ' obj.Cell.Name '")'];
            skl{6} = ';====================== Load Skill Functions ============================================';
            skl{7} = ['load("' skySkill.axlFunctionPath '")'];
            skl{8} = ';====================== Open Setup ============================================';
            skl{9} = '; Check that the test bench schematic view exists';
            skl{10} = ['schemCellObj = dbOpenCellViewByType("' obj.Cell.Library.UserLibraryName '" "' obj.Cell.Name '" "schematic")'];
            skl{11} = 'if(schemCellObj == nil ';
            skl{12} = sprintf('\tthen');
            skl{13} = sprintf('\t\terror("The schematic view must be created before the ADEXL view is imported.  Please make sure the test bench schematic and config views have been created and try importing the adexl view again.")');
            skl{14} = sprintf('\telse');
            skl{15} = sprintf('\t\tdbClose(schemCellObj));');
            skl{16} = '; Check that the test bench config view exists';
            skl{17} = ['unless(hdbIsConfig("' obj.Cell.Library.UserLibraryName '" "' obj.Cell.Name '" "config")'];
            skl{18} = sprintf('\terror("The config view must be created before the ADEXL view is imported.  Please make sure the test bench schematic and config views have been created and try importing the adexl view again."));');
            skl{19} = '; Open or create the adexl view';
            skl{20} = ['ddGetObj("' obj.Cell.Library.UserLibraryName '" "' obj.Cell.Name '" "adexl" "data.sdb" nil "w")']; % Create cellview
            skl{21} = 'sessionName = strcat("mysession" (sprintf nil "%d" random()))';
            skl{22} = 'axlSession = axlCreateSession(sessionName)';
%             sdb = axlNewSetupDBLCV("myLib" "myCell" "myView")
            skl{23} = ['sdb = axlSetMainSetupDBLCV( axlSession "' obj.Cell.Library.UserLibraryName '" "' obj.Cell.Name '" "adexl")'];
            skl{24} = '';
            skl{25} = '; Close an already open database and reopen it';
            skl{26} = 'sdb = if(sdb == nil ';
            skl{27} = sprintf('\tthen');
            skl{28} = sprintf('\t\tprintf("The adexl view cannot be opened, attempting to close and reopen it.") ');
            skl{29} = sprintf('\t\taxlCloseSetupDB( sdb ) ');
            skl{30} = sprintf(['\t\taxlSetMainSetupDBLCV( axlSession "' obj.Cell.Library.UserLibraryName '" "' obj.Cell.Name '" "adexl")']);
            skl{31} = sprintf('\telse');
            skl{32} = sprintf('\t\tsdb);');
            skl{33} = '; Throw an error if the adexl database still can''t be opened';
            skl{34} = 'unless(sdb';
            skl{35} = sprintf('\terror("The adexl cellview could not be opened.  Please make sure the cellview is closed, then restart cadence and try to load the script again."));');
            skl{36} = '';
            skl{37} = ';====================== Cellview Setup ============================================';
            skl{38} = '; Global Variables';
...
%             skl{end+1} = 'axlSetAllVarsDisabled(sdb 1)'; % Disable All Global Variables
            ...
            skl{end+1} = ['printf("COMPLETE: Loaded ADEXL Testbench for ' obj.Cell.Name '")'];
%             if(nargin == 1)
