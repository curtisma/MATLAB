...
%             obj.Paths.testData = 
        ...
        function skl = skill(obj,MipiStates)
            ...
            skl{1} = [';---------- Test "' obj.Name '" -------------'];
            skl{2} = ['printf(" Loading Test ' obj.Name '...")'];
            skl{3} = ['htest = axlPutTest( sdb "' obj.Name '" "ADE")'];
            skl{4} = ['axlSetTestToolArgs( htest list( list("lib" "' obj.Design.Library.UserLibraryName '") '...
                                                      'list("cell" "' obj.Design.Name '") '...
                                                      'list("view" "config") '...
                                                      'list("sim" "' obj.Simulator '"))) '];
            skl{5} = ['testSession = axlGetToolSession(axlSession "' obj.Name '")'];
            skl{6} = 'testSession = asiGetSession(testSession)';
