...
JST.example_card = _.template(
        ...
        '<div class="col-xs-6"><div class="card_action_secondary"><a href="<%= url %>">Read More</a></div></div>' +
...
function handleFeaturedExamples(productList) {
    ...
    if (productList.length > 0) {
        var compiledTmpl = JST.installedHspTmpl({installedhsps: productList});
        $('#all_product_list').append(compiledTmpl);
        $('#all_products').show();
        $('#related_resources').show();        
