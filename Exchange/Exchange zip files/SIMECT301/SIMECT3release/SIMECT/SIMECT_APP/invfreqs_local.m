function [b,a]=invfreqs_local(g,w,varargin)
...
%   $Revision: 1.7.4.4 $  $Date: 2008/12/04 23:20:47 $
...
case 'complex'
    realFlag = 0;
otherwise
    warning(generatemsgid('InvalidParam'),'String ''%s'' not recognized.  I assume you meant ''complex''.', realStr);
    realFlag = 0;
end
...
elseif (strcmp(pf,'trace')),
    ...
    error(generatemsgid('NotSupported'),['Trace flag ''' pf ''' not recognizable']);
end
if isempty(wf),wf=ones(length(w),1);end
wf=sqrt(wf);

if length(g)~=length(w),error(generatemsgid('InvalidDimensions'),'The lengths of H and W must coincide'),end
if length(wf)~=length(w),error(generatemsgid('InvalidDimensions'),'The lengths of Wt and W must coincide'),end
if any( w(:)<0 ) && realFlag
    warnStr = sprintf(...
    ...
    warning(generatemsgid('InvalidParam'),warnStr)
end
...
if (max(isinf(a))>0) || (max(isnan(a))>0)
   b=mean(g) ;
   a=1 ;
   return ;
end
