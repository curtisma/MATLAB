function parameters=validateInput2(argsIn,validParameters,force)
%VALIDATEINPUT  Validate input for m-files and set default value for each
% parameter
...
%       validParameters = {{{'print','p'},true},...
%                          {{'size','s'},10},{'name','Curtis'}};
...
while i<=numel(argsIn) % Do until the end of
    ...
    if i+1>numel(argsIn)||valid(argsIn{i+1},validParameters)
        ...
        % If the value is logical and true, set it to true.
