...
% interaction: %changed from the original version by Sathyan
pred_modePr = TransPr'*modePr;
MixPr = TransPr.*(modePr*(pred_modePr.^(-1))');
modex0 = modex*MixPr; modePP0 = modeP;modePP1=modeP;%%%%changed
for j=1:nm
  xk1=modex(:,j); PP=xk1*xk1'; modePP0(:,j)=PP(:);
  xk1=modex0(:,j); PP=xk1*xk1'; modePP1(:,j)=PP(:);%%%%changed
end
modeP = (modeP+modePP0)*MixPr-modePP1;%%%%changed
